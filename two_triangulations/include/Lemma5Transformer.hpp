...
namespace cgshop {
    ...
    public:
        ...
        Stats transform_from_xy_to_uv(int x, int u, int y, int v) {
            ...
            for (int i = pos_[y]; i != pos_[g]; i=(i+add)%n){
                poly.push_back(B_[i]);
            }
            poly.push_back(B_[pos_[g]]);
    ...
    private:
        ...
        inline void callLemma4(std::vector<int>& poly, int x1, int w, int y1){
            ...
            for (auto f : F_){
                bool ok =1;
                for (int i = 0; i < 3; ++i)
                    if (!std::binary_search(poly.begin(), poly.end(), f[i])){
                        ok = 0;
                        break;
                    }
                if (ok) {
