...
// ---------- Utility predicates ----------
        ...
        Stats introduce(int v1, int vq) {
            ...
            while (safe_guard-- > 0) {
                ...
                for (const auto& kv : edge2faces) {
                    ...
                    if (newCross) { ++reject_still_cross; continue; }

                    std::cout << "IS IT CONVEX???\n";
                    std::cout << a << ' ' << b << ' ' << x << ' ' << y <<'\n';
                    if (!isConvexQuadFlipOK(a,b,x,y)) { ++reject_nonconvex; continue; }
    ...
    private:
        ...
        bool quadConvex(int q0, int q1, int q2, int q3, bool flag = 0) const {
            ...
            return ccw(A,B,C, flag) && ccw(B,C,D, flag) && ccw(C,D,A) && ccw(D,A,B);
        }
        bool isConvexQuadFlipOK(int a, int b, int x, int y) const {
            ...
            if (a == 1 && b == 11 && x == 0 && y == 2)
                std::cout << "ALI " << quadConvex(x,b,y,a);
