...
def solveInstance(satSolverPath, executablePath, cnfDir, outDir, idxDir, assignmentDir,
  ...
  if m==2:
    ...
    distances=[-1]*m
    if instance_uid=="random_instance_552_320_20":
      distances=[-1, -1, -1, -1, -1, -1, -1, -1, -1, 5, 6, 5, 5, 6, 6, 7, 5, 5, 5, 6]
      i=8
    else:
      distances[m-1]=pairwiseDist[m-1][m-2]//2
      distances[m-2]=pairwiseDist[m-1][m-2]-distances[m-1]
      i=m-3
    while i>=0:
      distances[i]=0
      for j in range(i+1,m):
        distances[i]=max(distances[i],pairwiseDist[i][j]-distances[j])
      foundSol=False
      while not foundSol:
        print(instance_uid+" searching "+str(i)+" at pfd: "+str(distances[i]))
        foundSol=executeAndVerify(satSolverPath, executablePath, cnfDir, outDir, idxDir, assignmentDir, inFile, inDir, distances)
      i-=1

    totDist=sum(distances)
    if totDist<upperBoundDict[instance_uid]:
      print("Improved solution from pfd "+str(upperBoundDict[instance_uid])+" to pfd "+str(totDist))
      upperBoundDict[instance_uid]=low
      initialPathDict[instance_uid]=outDir+instance_uid+".solution.json"
      subprocess.run(["cp",initialPathDict[instance_uid],currentBestDir+instance_uid+".solution.json"])
