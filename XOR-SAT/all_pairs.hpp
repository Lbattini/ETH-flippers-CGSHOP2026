...
void formulate(Triangulation A, int k, string path, string edge_path, string input_path){
    ...
    if (!qs.empty())
        second++;
    else if (filesystem::exists("C:\\libs\\cryptominisat5-win\\quads\\" + input_path + ".txt")){
        int lenqs;
        ifstream quads("C:\\libs\\cryptominisat5-win\\quads\\" + input_path + ".txt");
        ...
        qs = findAllQuadrilateralsFast(A);
        ofstream quads("C:\\libs\\cryptominisat5-win\\quads\\" + input_path + ".txt");
    ...
    vector<vector<int>> f(nqs, vector<int>(k+1));
    for (int i = 1; i <= k; ++i){
        for (int j = 0; j < nqs; ++j){
            f[j][i] = cur++;
    ...
    vector<vector<vector<int>>> e(A.n, vector<vector<int>>(A.n, vector<int>(k+1)));
    for (int i = 0; i < A.n; ++i){
    ...
    for (int u = 0; u < A.n; ++u){
        for (int v = u+1; v < A.n; ++v){
            ...
            for (int t = 1; t <= k; ++t){
                vector<int> a = {e[u][v][t-1], e[u][v][t]};
                    ...
                    if (len <= 3) {
                        for (int i = 0; i < len; ++i) {
                            for (int j = i + 1; j < len; ++j) {
                                ...
                                out_buf += to_string(-f[conf_qs[i]][t]) + "  " + std::to_string(-f[conf_qs[j]][t]) + " 0\n";
                                clause_cnt++;
                            }
                        }
                    } else {
                        ...
                        for (int i = 0; i < len; ++i) {
                            ...
                            out_buf += to_string(-f[conf_qs[i]][t]) + "  " + to_string(state[i]) + " 0\n";
                            clause_cnt++;

                            if (i > 0) {
                                //clause(-f[conf_qs[i]][t], -state[i - 1]);
                                //clause(-state[i - 1], state[i]);
                                out_buf += to_string(-f[conf_qs[i]][t]) + "  " + to_string(-state[i-1]) + " 0\n";
