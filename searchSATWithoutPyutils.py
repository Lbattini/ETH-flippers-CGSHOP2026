...
def executeAndVerify(satSolverPath, executablePath, cnfDir, outDir, idxDir, assignmentDir, inFile, inDir, distances):
  ...
  if satSolverPath=="./build/mallob":
    ...
    subprocess.run([satSolverPath,"-mono="+cnfFile,"-t=16","-satsolver=k","-s2f="+assignmentFile],capture_output=False)
    os.chdir("../cgshop2026/")
  else:
...
def searchDistanceList(satSolverPath,executablePath,cnfDir, outDir, idxDir, assignmentDir, inFile, inDir,distanceList):
  distListWithKey=[]
  for distance in distanceList:
    distListWithKey.append([max(distance)]+distance)
  distListWithKey.sort(key=lambda distances : distances[0])
  j=0
  for distWithKey in distListWithKey:
    distances=distWithKey[1:]
    print(str(j)+"/"+str(len(distListWithKey)-1))
    j+=1
    print(distances)
  ...
  if m==2:
    ...
    if os.path.isfile(minFromFileName):
      ...
      if instance_uid=="random_instance_213_80_20":
        ...
        # for suffixStart in range(m-3,startMinFrom-1,-1):
        #   # print("Suffix start: "+str(suffixStart))
        #   low1=minFrom[suffixStart+1]
        #   minFrom[suffixStart]=binSearchSuffixFindHigh(satSolverPath,executablePath,cnfDir,outDir,idxDir,assignmentDir,inDir,inFile,pairwiseDist,low1,m,minFrom, suffixStart)
...
if __name__ == '__main__':
  ...
  maxM=0
  cntSolved=0
  #executeAndVerify(satSolverPath, executablePath, cnfDir, outDir, idxDir, assignmentDir, "woc-175-tsplib-c6fb5178.json", inDir, [3,8,11,13,5,14])
  #executeAndVerify(satSolverPath, executablePath, cnfDir, outDir, idxDir, assignmentDir, "woc-230-tsplib-32634ca5.json", inDir, [8,10,11,7,9,11])
  #executeAndVerify(satSolverPath, executablePath, cnfDir, outDir, idxDir, assignmentDir, "woc-230-tsplib-32634ca5.json", inDir, [8,10,11,8,8,11])
  #executeAndVerify(satSolverPath, executablePath, cnfDir, outDir, idxDir, assignmentDir, "woc-230-tsplib-32634ca5.json", inDir, [8,10,11,9,7,11])
  ...
  print(instancesToSolve)
  nThreads=min(16,max(1,len(instancesToSolve)))
  print("Number of threads: "+str(nThreads))
  cntSolved=0
  with Pool(nThreads) as p:
    partialFunc=partial(solveInstance,satSolverPath,executablePath,cnfDir,outDir,idxDir,assignmentDir,inDir,pairwiseDistDir,lowerBoundDict,upperBoundDict)
    # partialFunc(instancesToSolve[0])
    resIt=map(partialFunc,instancesToSolve)
    # resIt=p.imap_unordered(partialFunc,instancesToSolve)
    for r in resIt:
      # print("Solved an instance")
      cntSolved+=1
